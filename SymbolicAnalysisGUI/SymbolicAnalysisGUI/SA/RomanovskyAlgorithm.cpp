...
RomanovskyAlgorithm::RomanovskyAlgorithm(const float& left, const float& right, const float& top, const float& bot,
    ...
    Mapping(formulaX, formulaY, arcWeightFormula) {};

// Экстремальные циклы (минимальный и максимальный)
void RomanovskyAlgorithm::RomanovskyAlgorithmCalculation(unordered_map<int, unordered_set<int>>& baseGraph)
{
    minCircuits.clear();
    maxCircuits.clear();
    ...
    vector<vector<int>> _strongComponents = getStrongComponents(baseGraph);
    ...
    unordered_map<int, unordered_set<int>> graph = getGraphWithoutSelfLoops(baseGraph);
    ...
    for (auto& SC : strongComponents)
    {
        for (auto& cell : SC)
        {
            cellToWeight[cell] = getArcWeightForMap(cell, *baseGraph[cell].begin(), 1.0f);
        }
    }
    ...
    for (auto& SC : strongComponents)
    {
        ...
        for (auto& cell : SC)
        {
            if (inContainer(cell, baseGraph[cell]))
            {
...
unordered_map<int, int> RomanovskyAlgorithm::getStartCircuit(unordered_map<int, unordered_set<int>>& graph, const int& multiplier)
{
...
void RomanovskyAlgorithm::runAlgorithmForConstructingTreeAndOptimalContour(unordered_map<int, int>& circuit, const int& multiplier, unordered_map<int, unordered_set<int>>& graph)
{
...
// Получаем (x, y) из которых бы попали из vertex в linkedVertex
void RomanovskyAlgorithm::getArcStartXY(float& xW, float& yW, const int& cell, const int& linkedCell)
{
    float leftX, topY;
...
float RomanovskyAlgorithm::getArcWeight(const int& vertex, const int& linkedVertex, const float& multiplier)
{
    return multiplier * cellToWeight[vertex];
}

float RomanovskyAlgorithm::getArcWeightForMap(const int& vertex, const int& linkedVertex, const float& multiplier)
{
    float x, y;
    getArcStartXY(x, y, vertex, linkedVertex);
    return multiplier * calculateArcWeight(x, y);
}

float RomanovskyAlgorithm::getCircuitWeight(unordered_map<int, int>& circuit, const float& multiplier)
{
    float weight = 0;
    for (auto& keyValue : circuit)
    {
...
vector<vector<int>>& RomanovskyAlgorithm::getMinCircuits()
{
    return minCircuits;
}

vector<vector<int>>& RomanovskyAlgorithm::getMaxCircuits()
{
    return maxCircuits;
}

vector<float>& RomanovskyAlgorithm::getMinCircuitsWeights()
{
    return minCircuitsWeights;
}

vector<float>& RomanovskyAlgorithm::getMaxCircuitsWeights()
{
    return maxCircuitsWeights;
}

vector<int>& RomanovskyAlgorithm::getSCorder()
{
    return SCorder;
}
