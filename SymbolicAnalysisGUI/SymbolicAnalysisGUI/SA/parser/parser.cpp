...
EndOfExpressionExpected[] = "End of expression expected",
OpenBracket[] = "'(' or '[' expected",
ClosingBracket[] = "')' or ']' expected";

// Member functions
TParserNode *TParser::CreateNode(float _value, TParserNode *_left, TParserNode *_right)
{
...
TParserNode *TParser::Expr5(void)
{
   ...
   {
      case PARSER_NUMBER:
         temp = CreateNode((float)atof(curToken));
         GetToken();
         break;

      case PARSER_PI:
         temp = CreateNode((float)M_PI);
         GetToken();
         break;

      case PARSER_E:
         temp = CreateNode((float)exp(1));
         GetToken();
         break;
...
float TParser::Evaluate(void)
{
   result = CalcTree(root);
   return result;
}

float TParser::CalcTree(TParserNode *tree)
{
   static float temp;
   ...
   if(tree->left==NULL && tree->right==NULL)
      ...
      switch((int)tree->value)
      {
         ...
         case OP_POWER:
            return (float)pow(CalcTree(tree->left),CalcTree(tree->right));
